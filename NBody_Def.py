...
class Object:
    ...
    def __init__(self, name="Object", mass=10.0,radius=1.0,
...
class System:
    ...
    Sysforces = np.zeros(shape=(1, 1), dtype=float)
    System = []
    dt = 0.01       # time step
    Tt = 100        # total time
    tt = 0          # current time
    ...
    def usersys(self):
        ...
        self.Sysforces = np.zeros(shape=(self.size, self.size, 1, 3), dtype=float)
    ...
    def samplesys(self):
        ...
        self.Sysdist = np.zeros(shape=(self.size, self.size), dtype=float)
        self.Sysforces = np.zeros(shape=(self.size, self.size, 1, 3), dtype=float)
#        self.System.append(Object(name="Prospero", mass=100.0, radius=1.0, position=[10.0, 10.0, 0.0], velocity=[0.0, 0.0, 0.0]))
        self.System.append(Object(name="Caliban", mass=100.0, radius=1.0, position=[10.0, 0.0, 0.0], velocity=[0.0, 0.0, 0.0]))
#        self.System.append(Object(name="Ariel", mass=100.0, radius=1.0, position=[0.0, 0.0, 0.0], velocity=[0.0, 0.0, 0.0]))
        self.System.append(Object(name="Miranda", mass=100.0, radius=1.0, position=[0.0, 10.0, 0.0], velocity=[0.0, 0.0, 0.0]))
#        self.System.append(Object(name="Sycorax", mass=100.0, radius=1.0, position=[5.0, 5.0, 0.0], velocity=[0.0, 0.0, 0.0]))
        self.Tt = 10000
        self.dt = 1
        self.tt = 0.0
        self.totalsteps = self.Tt/self.dt
    ...
    def startstate(self):
        for i in range(self.size):
            for j in range(self.size):
                ...
                self.Sysforces[i, j, 0, :] = (self.G * self.System[i].mass * self.System[j].mass /      # calc F(i,j)
                               self.Sysdist[i][j]**2)
        for i in range(self.size):
            temp = np.zeros(shape=(1, 3), dtype=float)
            for j in range(self.size):
                temp += self.Sysforces[i, j, 0, :]
            self.System[i].netforce = temp              # calc Fnet(i)
